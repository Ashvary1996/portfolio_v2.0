...
const techIcons: Record<string, any> = {
  ...
  restfulApi: <Image src={restApiIcon} alt="REST API" />,
...
const Projects = () => {
  const [viewMoreIndex, setViewMoreIndex] = useState<number | null>(null);

  const handleToggleViewMore = (index: number) => {
    ...
    >
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-10">
          <h2 className="text-3xl font-bold  mb-3 text-shadow">Projects</h2>
          <p className="italic ">A preview of some projects I worked on.</p>
        </div>
        ...
        <fieldset className="border border-gray-300 rounded-lg p-6">
          ...
          <div className="space-y-8">
            {majorProjects.map((project, index) => (
              <div
                key={index}
                className={`project-card p-6     rounded-lg shadow-md transition-transform duration-500 hover:transform hover:scale-105 hover:shadow-lg flex flex-col lg:flex-row ${
                  index % 2 === 0 ? "" : "lg:flex-row-reverse"
                }`}
              >
                <div className="lg:w-3/5 mb-6 lg:mb-0 ">
                  <h3 className="text-2xl font-bold mb-2 hover:text-red-400">
                    {project.title}
                  </h3>
        ...
        {/* <fieldset className="border border-gray-300 rounded-lg p-6 mt-10">
          <legend className="   text-2xl mb-6">Mini Projects</legend>

          <div className="flex flex-wrap justify-center gap-5">
            {miniProjects.map((project, index) => (
              <div
                key={index}
                className="relative flex flex-col w-full sm:w-1/2 md:w-1/3 lg:w-1/4 p-4 overflow-hidden rounded-lg    shadow-md transition-transform duration-300 hover:transform hover:scale-105 hover:shadow-lg"
              >
                <h4 className="  text-lg lg:text-xl font-semibold mb-2 text-center">
                  {project.title}
                </h4>
                <Image
                  ...
                  alt={project.title}
                />
                <div className="absolute top-0 left-0 w-full h-full flex flex-col items-center justify-center text-white bg-gray-900/80 bg-opacity-75 opacity-0 transition-opacity duration-300 ease-in-out hover:opacity-100 p-4">
